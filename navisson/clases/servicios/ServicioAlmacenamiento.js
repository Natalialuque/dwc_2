...
// Importamos las funciónes de cookies
...
// Cookie que recuerda la última pantalla pulsada para modificar
const COOKIE_ULTIMA_PANTALLA = "navisson.ultimaPantallaModificada";
// Cookie que recuerda si el fondo oscuro está activo
const COOKIE_FONDO_OSCURO = "navisson.fondoOscuro";

// Objeto que agrupa las funciónes de almacenamiento
export const ServicioAlmacenamiento = {
    ...
    // Carga pantallas desde localStorage
    cargarPantallas() {
        // Leemos el texto guardado
        const guardadas = localStorage.getItem(CLAVE_LOCAL);

        // Si no hay nada guardado, Devolvemos array vacío
        if (!guardadas) {
            return [];
        }
        ...
        try {
            ...
            // Si hay error, avisamos y Devolvemos array vacío
    ...
    cargarFiltros() {
        ...
        // Si no hay filtros, Devolvemos valores por defecto
        if (!guardados) {
        ...
        try {
            ...
            // Si falla, Devolvemos filtros por defecto
    ...
    guardarFondoOscuro(activado) {
        // Guardamos si/no para que sea fácil leerlo
