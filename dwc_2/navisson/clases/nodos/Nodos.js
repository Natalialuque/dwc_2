/**
 * Clase encargada de crear los nodos 
 *  */ 
export class Nodos {
    // Recibimos el id del contenedor donde se montara toda la aplicacion
    constructor(idContenedor = "aplicacion") {
        // Guardamos el contenedor raiz para poder anadir dentro todos los nodos
    ...
    crearElemento(etiqueta, clase = "") {
        ...
        // Si nos pasan una clase, se le asignamos al elemento
        if (clase) {
    ...
    // Crea un botn normal de la aplicacion
    crearBoton(texto, clase, id = "") {
        // Creamos el boton con createElement
        const boton = this.crearElemento("button", clase);

        // Todos los botpnes normales son type button para que no envien formularios sin querer
        boton.type = "button";
    ...
    crearTituloSeccion(etiquetaTexto, tituloTexto, idTitulo = "") {
        ...
        // Si necesitamos localiar el titulo desde main, le ponemos id
        if (idTitulo) {
            titulo.id = idTitulo;
        }
    ...
    crearCampoTexto(id, nombre, textoLabel, tipo, placeholder, idError, requerido = true) {
        ...
        // El precio usa teclado decimal en movil
    ...
    crearCampoArchivo(id, nombre, textoLabel, accept) {
        ...
        // Devolvemos contenedor
        return contenedor;
    }

    // Crea un select con label, opciones y erro
    crearSelect(id, nombre, textoLabel, opciones, idError) {
        ...
        // Recorremos las opciones recibidas y las anadimos
    ...
    // Crea toda la estructura base de la aplicacion dentro del contenedor raiz
    crearAplicacion() {
        // Limpiamos el contenedor por si se llama mas de una vez
    ...
    crearAcceso() {
        // Capa que tapa la aplicacion hasta validar usuario
        ...
        const boton = this.crearBoton("Iniciar sesion", "boton-principal", "botonIniciarSesion");
        ...
        // Configuramos contraseña, sin ñ porque daba error
        contrasena.id = "contrasenaAcceso";
        contrasena.name = "contrasena";
        contrasena.type = "password";
        contrasena.placeholder = "Contrasena";
        contrasena.autocomplete = "current-password";
        contrasena.required = true;

        // Configuramos error y boton
        ...
        // Lo añadimos al body para que coja toda la ventana
    ...
    crearFormulario() {
        ...
        // Primera fila de tamano y precio
        ...
        // El boton guardar dbe enviar el formulario
        guardar.type = "submit";
    ...
    // Devuelve la clase CSS segun el estado de la pantalla
    obtenerClaseEstado(estado) {
        // Si esta en stock usamos clase verde
        if (estado === "En stock") {
            return "stock";
        }

        // Si esta enviada usamos clase naranja
        if (estado === "Enviada") {
            return "enviada";
        }
